...
cronJobs:
  ...
  - id: workflow-runner-loop
    ...
    message: |
      ...
      Command:
        hermes recipes workflows runner-tick --team-id {{teamId}} --concurrency 2 --lease-seconds 45
...
templates:
  ...
  lead.backupWork: |
    ...
    ROOT="$HOME/.hermes/workspace-{{teamId}}"
    OUTDIR="$HOME/.hermes/workspace/_backups"
    mkdir -p "$OUTDIR"
  ...
  test.agents: |
    ...
    1) Remove test workspaces:
       ```bash
       rm -rf ~/.hermes/workspace-<test-team-id>
       ```

    2) Remove test agents from config (agents whose id starts with the test team id):
       - Edit `~/.hermes/config.yaml` and remove entries from `agents.list[]`
       - Or wait for `hermes recipes remove-team` (once available)

    3) Remove any cron jobs created for the test team:
       ```bash
       hermes cron list --all --json | grep "<test-team-id>"
       hermes cron remove <jobId>
       ```
...
## What you get
- Shared workspace at `~/.hermes/workspace-<teamId>/` (e.g. `~/.hermes/workspace-development-team-team/`)
- File-first tickets: backlog → in-progress → testing → done
- Team lead acts as dispatcher; tester verifies before done

## Files
- Creates a shared team workspace under `~/.hermes/workspace-<teamId>/` (example: `~/.hermes/workspace-development-team-team/`).
